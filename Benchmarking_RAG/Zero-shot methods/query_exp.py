...
def with_initial_response(query, initial_response, collection, openai_api_key): 
    ...
    rag_text = collection.query(query_texts = [query, initial_response], n_results=5)

    rag_context = ""
    for metadata_pair in rag_text['metadatas']:
        for metadata in metadata_pair:
            rag_context += f"{metadata['sentence']}\n"

    print("Context retreived:", rag_context)
    system_prompt = """You are a financial chatbot trained to answer questions based on the information provided in 10-K
        documents. Your responses should be directly sourced from the content of these documents."""

    query_prompt = f"Question: {query}. Relevant document information: {rag_context}"
    messages = [
        {"role": "system", "content": system_prompt},
...
def main():
    ...
    for index, row in test.iterrows():
        ...
        initial_response = no_context(question, openai_api_key)
        final_response = with_initial_response(question, initial_response, collection, openai_api_key)
        ...
        llm_eval = evaluate_llm_responses(question, final_response, ref_answer, openai_api_key)
    ...
    results_df.to_csv('query_exp_results_test.csv', index=False)
