...
def rerank_and_create_context(query, collection, openai_api_key, cross_encoder_model="cross-encoder/ms-marco-TinyBERT-L-2-v2", initial_top_k=10, final_top_k=3):
    """
    Reranks the retrieved chunks and combines them into a new context.
    """
    try:
        openai_ef = embedding_functions.OpenAIEmbeddingFunction(api_key=openai_api_key)
        query_vector = openai_ef(query)
        
        # Step 1: Retrieve initial chunks from the collection
        results = collection.query(query_texts=[query], n_results=initial_top_k)
        
        # Step 2: Extract content from initial results
        initial_chunks = []
        for metadata_pair in results['metadatas']:
            for metadata in metadata_pair:
                initial_chunks.append(metadata['sentence'])

        # Step 3: Initialize CrossEncoder for reranking
        cross_encoder = CrossEncoder(cross_encoder_model, max_length=512, device="cpu")

        # Step 4: Rerank the chunks
        ...
        # Step 5: Extract top reranked chunks
        top_chunks = [initial_chunks[idx] for idx in sorted_indices[:final_top_k]]

        # Step 6: Create context string by joining the top chunks
        ...
        print(f"Error during reranking and creating context: {e}")
        return ""
...
def main():
    ...
    for index, row in test.iterrows():
        ...
        reranked_docs = rerank_and_create_context(question, collection, openai_api_key=openai_api_key)
        print("Querying Model now")
        model_answer = query_openai_with_context(question, context = reranked_docs, openai_api_key=openai_api_key)
        print(model_answer)
    ...
    results_df.to_csv('reranker_test_results_test.csv', index=False)
